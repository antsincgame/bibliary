 ...
 * stream directly to vectordb). So pause = abort, discard = no-op,
...
/**
 * No-op checkpoint store. Extraction state lives in vectordb (accepted
  ...
  const handle: PipelineHandle = {
    ...
    flushPending: async () => {
      /* Nothing buffered locally. Each accepted concept is upserted
         immediately into vectordb; the embedder cache flushes on its own. */
    },
  };
