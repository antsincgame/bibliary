 ...
 *
 * МОТИВАЦИЯ:
 *   Локальный LM Studio — один процесс, один пул VRAM/RAM. Если фоновый Arena
 *   cycle стартует во время массового импорта DJVU/PDF (vision-meta inline в
 *   importBookFromFile) или пока evaluator queue гоняет pre-flight scoring,
 *   у LM Studio либо OOM, либо таймауты, либо — что хуже всего — обе очереди
 *   конкурируют за один и тот же model handle и режут друг друга.
 *
 * АРХИТЕКТУРА:
 ...
 *     2. Любой "тяжёлый" фоновый job (сегодня — Arena scheduler) перед
 *        стартом вызывает `globalLlmLock.isBusy()`. Если хоть один probe
 *        busy — job скипает свой тик и пишет skip-метрику.
 *     3. Pro UI читает `getStatus()` через IPC `arena:get-lock-status` и
 *        показывает что мешает калибровке (или "idle, ready").
 *
 *   Никаких mutex-семантик с acquire/release — у нас не критическая секция
 *   с гонкой за один ресурс, у нас "вежливый" фоновый воркер, который НЕ
