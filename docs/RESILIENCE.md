# Bibliary Resilience Layer (v3.1.0)
...
Resilience layer покрыт через основной unit-suite. Скрипты вида `scripts/test-platform.ts`, `test-token.ts`, `test-resume-batch.ts` и т.п. **не существуют** в репозитории. Используйте:

```bash
# Основной unit-suite (покрывает async-pool, archive-tracker, batch-runner)
npm run test:fast

# Отдельные тест-файлы релевантные для resilience:
# tests/batch-runner.test.ts       — runBatchExtraction, error recovery
# tests/async-pool.test.ts         — runWithConcurrency
# tests/archive-tracker.test.ts    — ArchiveTracker register/finishOne
# tests/library-pipeline-contract.test.ts — pipeline contract

# Полный пересборка native + тесты
npm test

# Electron smoke (включает bootstrap и IPC bridge)
npm run test:smoke
```

Acceptance gate: `npm run lint` + `npm run test:fast` + `npm run electron:compile` + `npm run test:smoke` — всё exit 0.

---

## Регистрация нового pipeline

Чтобы добавить новый длительный pipeline (реальный пример — `electron/lib/dataset-v2/coordinator-pipeline.ts`):

```ts
// electron/lib/dataset-v2/coordinator-pipeline.ts
import { coordinator, type PipelineHandle } from "../resilience/index.js";
...
LM Studio v0.4.12 игнорировал `chat_template_kwargs.enable_thinking`. В LM Studio 0.4.12+ это поведение улучшено. Bibliary выставляет `enable_thinking: false` автоматически через `chatTemplateKwargs`. Если модель всё равно генерирует `<think>` — повысьте `max_tokens` для JSON-режима (6144/4096/3072 для T1/T2/T3 — хватает на reasoning + answer для Qwen3.x).
...
**Версия документации:** v3.1.0 · соответствует `electron/lib/resilience/` (10 файлов: atomic-write, batch-coordinator, bootstrap, checkpoint-store, constants, file-lock, index, lm-request-policy, lmstudio-watchdog, telemetry).
