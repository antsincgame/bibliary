# Дообучение — мастер fine-tuning (Self-Hosted, v3.1.0)

> v3.1.0 · «датасет → fine-tune за 3 шага, на своём железе, без облачных счетов»
>
...
Тесты Forge интегрированы в unit-suite. Отдельного `scripts/test-forge.ts` нет. Покрытые модули:

- `tests/dataset-synth-presets.test.ts` — пресеты синтеза и форматирование
- `tests/library-pipeline-contract.test.ts` — контракт pipeline

Ключевые компоненты (`format.ts`, `configgen.ts`, `pipeline.ts`) тестируются через smoke. Для полного покрытия — см. P2.6 в `docs/ROADMAP-TO-MVP.md`.

## Статус Phase 3.3

- WSL detector + auto-bootstrap `electron/defaults/forge/setup-wsl.sh` ✅ **готово**
- `wsl python forge-train.py` с live loss/grad-norm стримом ✅ **LocalRunner** готов
- Auto-copy GGUF → LM Studio dir ✅ **готово** (`importGgufToLMStudio`)
- Eval Harness (ROUGE-L + LLM-as-judge) ✅ **готов**
- Автоматическая регистрация в ProfileStore после GGUF — **backlog**
- Multi-run experiment matrix (N spec параллельно) — **backlog**
