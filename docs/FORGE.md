# Forge — Cloud Bridge fine-tune wizard (Phase 3.2)

> v2.3.0 · «датасет → fine-tune за 4 шага без Python»

## Что это

Forge — это `route` в Bibliary, который превращает ShareGPT/ChatML JSONL батч в production-ready bundle для fine-tune в любом популярном фреймворке 2026:
...
Внизу — **Memory Forge embedded slider** (для `max_seq_length`) + **VRAM Calculator** (предсказывает поместится ли).
...
Forge зарегистрирован как `pipeline: "forge"` в `coordinator`. Это даёт:
...
- **Graceful shutdown**: forge state сохраняется в `data/forge/checkpoints/<runId>.json` через `createCheckpointStore`
