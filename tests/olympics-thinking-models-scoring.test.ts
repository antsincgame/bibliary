...
/* ─── v1.0.12 BUG-FIX: scorer Колмогорова требует обоснование ────────── */

test("v1.0.12: evaluator-ru-thinking-kolmogorov score=10 БЕЗ reasoning → подозрительный (≤0.25)", () => {
  /* Регрессионный тест: до v1.0.12 score=10 + reasoning="" давал 0.45 балла,
   * что позволяло thinking-моделям проходить тест без обоснования. */
  const d = getDiscipline("evaluator-ru-thinking-kolmogorov");
  const noReasoning = `{"score":10,"reasoning":""}`;
  const score = d.score(noReasoning);
  assert.ok(
    score <= 0.25,
    `score=10 без reasoning должен быть ≤ 0.25 (multi-criteria закрепление), got ${score}`,
  );
});

test("v1.0.12: evaluator-ru-thinking-kolmogorov score=10 с КОРОТКИМ reasoning → частичный (0.25-0.40)", () => {
  const d = getDiscipline("evaluator-ru-thinking-kolmogorov");
  /* Длина 26 символов, есть anchor "foundational" → попадаем в "частичный" branch */
  const shortReasoning = `{"score":10,"reasoning":"foundational classic work"}`;
  const score = d.score(shortReasoning);
  assert.ok(
    score >= 0.25 && score <= 0.50,
    `score=10 с коротким reasoning (26 chars + 1 anchor) → 0.25-0.50, got ${score}`,
  );
});

test("v1.0.12: evaluator-ru-thinking-kolmogorov score=10 с ПОЛНЫМ multi-criteria reasoning → высокий балл", () => {
  const d = getDiscipline("evaluator-ru-thinking-kolmogorov");
  const fullReasoning = `{"score":10,"reasoning":"Колмогоров 1933 заложил аксиоматические основы теории вероятностей через теорию меры. До сих пор стандарт. Узкая область, нотация местами архаична."}`;
  const score = d.score(fullReasoning);
  assert.ok(
    score >= 0.85,
    `score=10 с полным multi-criteria reasoning + всеми anchors → score ≥ 0.85, got ${score}`,
  );
});

test("v1.0.12: evaluator-ru-thinking-kolmogorov score=10 с длинным reasoning БЕЗ anchors → подозрительный", () => {
  const d = getDiscipline("evaluator-ru-thinking-kolmogorov");
  /* 70+ chars длины, но все anchors отсутствуют (нет foundational/narrow/факты) */
  const noAnchors = `{"score":10,"reasoning":"очень хорошая книга которая мне понравилась полностью"}`;
  const score = d.score(noAnchors);
  assert.ok(
    score <= 0.31, /* IEEE-754: 0.20 + 0.10 = 0.30000000000000004 */
    `score=10 без anchors → попадаем в "подозрительный" branch (0.20) + 0.10 (length≥40) ≈ 0.30, got ${score}`,
  );
});

/* ═══════════════════════════════════════════════════════════════════════
 * v1.0.12 SOTA disciplines — scorer unit tests (Omnissiah audit fix)
 * ═══════════════════════════════════════════════════════════════════════ */

test("v1.0.12: crystallizer-hallucination-calibration — пустые массивы (оба поля) → score=1.0", () => {
  const d = getDiscipline("crystallizer-hallucination-calibration");
  const perfect = `{"facts":[],"entities":[]}`;
  const score = d.score(perfect);
  assert.ok(
    score >= 0.95,
    `Пустые facts+entities (нет галлюцинаций) → score ≥ 0.95, got ${score}`,
  );
});

test("v1.0.12: crystallizer-hallucination-calibration — только facts=[] без entities → неполный JSON (<1.0)", () => {
  const d = getDiscipline("crystallizer-hallucination-calibration");
  const partial = `{"facts":[]}`;
  const score = d.score(partial);
  assert.ok(
    score < 0.95 && score >= 0.60,
    `Только facts:[] без entities → неполный JSON, score 0.60-0.94, got ${score}`,
  );
});

test("v1.0.12: crystallizer-hallucination-calibration — галлюцинация (2+ фейковых facts) → низкий score", () => {
  const d = getDiscipline("crystallizer-hallucination-calibration");
  const hallucinated = `{"facts":["oxidative phosphorylation observed","enzyme catalysis detected"],"entities":[{"name":"molybdenum oxidoreductase","type":"enzyme"}]}`;
  const score = d.score(hallucinated);
  assert.ok(
    score <= 0.20,
    `3 галлюцинации + domain-bridge (enzyme, phosphorylat) → score ≤ 0.20, got ${score}`,
  );
});

test("v1.0.12: crystallizer-ru-medicine-semmelweis — полная extraction → высокий score", () => {
  const d = getDiscipline("crystallizer-ru-medicine-semmelweis");
  const full = JSON.stringify({
    facts: [
      "В 1847 году Земмельвейс установил связь между трупными частицами и послеродовой горячкой",
      "Мытьё рук хлорной известью снизило смертность с 18% до 1.3%",
      "Медицинское сообщество отвергло его идеи",
      "Пастер подтвердил микробную теорию в 1860-х",
      "Листер основал антисептическую хирургию в 1867",
    ],
    entities: [
      { name: "Земмельвейс", type: "person" },
      { name: "Венская больница", type: "institution" },
      { name: "Пастер", type: "person" },
      { name: "Листер", type: "person" },
      { name: "Браун", type: "person" },
      { name: "послеродовая горячка", type: "disease" },
    ],
    relations: [
      { subject: "Земмельвейс", predicate: "установил", object: "связь между заражением и смертностью" },
      { subject: "Листер", predicate: "основал", object: "антисептическую хирургию" },
    ],
  });
  const score = d.score(full);
  assert.ok(
    score >= 0.70,
    `Полная extraction Земмельвейса (5 facts, 6 entities, anchors) → score ≥ 0.70, got ${score}`,
  );
});

test("v1.0.12: crystallizer-ru-physics-mendeleev — полная extraction → высокий score", () => {
  const d = getDiscipline("crystallizer-ru-physics-mendeleev");
  const full = JSON.stringify({
    facts: [
      "В 1869 году Менделеев опубликовал периодическую таблицу",
      "Систематизировал 63 элемента по атомному весу",
      "Предсказал свойства экаалюминия, открытого в 1875 как галлий",
      "Экабор — скандий, открыт в 1879",
      "Экасилиций — германий, открыт в 1886",
      "Лотар Мейер составил аналогичную классификацию без предсказаний",
    ],
    entities: [
      { name: "Менделеев", type: "person" },
      { name: "Санкт-Петербург", type: "location" },
      { name: "галлий", type: "element" },
      { name: "скандий", type: "element" },
      { name: "германий", type: "element" },
      { name: "Мейер", type: "person" },
      { name: "периодический закон", type: "concept" },
    ],
    relations: [
      { subject: "Менделеев", predicate: "опубликовал", object: "периодическую таблицу" },
      { subject: "Менделеев", predicate: "предсказал", object: "свойства неоткрытых элементов" },
      { subject: "Мейер", predicate: "составил", object: "аналогичную классификацию" },
    ],
  });
  const score = d.score(full);
  assert.ok(
    score >= 0.70,
    `Полная extraction Менделеева (6 facts, 7 entities, 12 anchors) → score ≥ 0.70, got ${score}`,
  );
});

test("v1.0.12: crystallizer-ru-physics-mendeleev — 'лаборатория' НЕ штрафуется как 'бор'", () => {
  const d = getDiscipline("crystallizer-ru-physics-mendeleev");
  const withLab = JSON.stringify({
    facts: [
      "Менделеев работал в лаборатории Петербургского университета в 1869 году",
      "Опубликовал периодическую таблицу с предсказаниями галлия и скандия",
    ],
    entities: [
      { name: "Менделеев", type: "person" },
      { name: "лаборатория", type: "place" },
    ],
    relations: [],
  });
  const score = d.score(withLab);
  const withoutLab = JSON.stringify({
    facts: [
      "Менделеев работал в Петербургском университете в 1869 году",
      "Опубликовал периодическую таблицу с предсказаниями галлия и скандия",
    ],
    entities: [
      { name: "Менделеев", type: "person" },
  ...
  const scoreWithout = d.score(withoutLab);
  assert.ok(
    Math.abs(score - scoreWithout) < 0.06,
    `'лаборатория' не должна штрафоваться как 'Бор/bohr': разница ≤ 0.05, got ${Math.abs(score - scoreWithout).toFixed(3)}`,
  );
});

test("v1.0.12: vision_ocr-ru-math-hierarchical зарегистрирована в OLYMPICS_DISCIPLINES", () => {
  const d = OLYMPICS_DISCIPLINES.find((x) => x.id === "vision_ocr-ru-math-hierarchical");
  assert.ok(d, "vision_ocr-ru-math-hierarchical должна существовать");
  assert.strictEqual(d!.role, "vision_ocr");
  assert.ok(typeof d!.score === "function");
});

test("v1.0.12: vision_ocr-ru-math-hierarchical — полный recall с math symbols → высокий composite", () => {
  const d = getDiscipline("vision_ocr-ru-math-hierarchical");
  const fullOcr =
    "Для понимания книги достаточно знакомство с курсов математической программой " +
    "линейной алгебры, сведения о дифференциальных уравнениях и обозначения " +
    "теории множеств: объединение A ∪ B, пересечение A ∩ B, элемент x ∈ A, " +
    "подмножество A ⊂ B, декартово произведение A × B. " +
    "Функция f: E₁ → E₂ отображение значение аргумент. " +
    "Автор благодарен рецензирование рукописи. " +
    "Березину, Кудрявцеву, Федорюку. " +
    "Анализ курсов факультетах.";
  const score = d.score(fullOcr);
  assert.ok(
    score >= 0.70,
    `Полный OCR с math symbols + кириллица + авторы → composite ≥ 0.70, got ${score}`,
  );
});

test("v1.0.12: vision_ocr-ru-math-hierarchical — NO_TEXT → 0", () => {
  const d = getDiscipline("vision_ocr-ru-math-hierarchical");
  assert.strictEqual(d.score("NO_TEXT"), 0, "NO_TEXT когда текст есть → 0");
});

test("v1.0.12: vision_ocr-ru-math-hierarchical — markdown fences → штраф ×0.45", () => {
  const d = getDiscipline("vision_ocr-ru-math-hierarchical");
  const withFences = "```\nанализ множеств функция отображение ∪ ∩\n```";
  const withoutFences = "анализ множеств функция отображение ∪ ∩";
  const scoreFenced = d.score(withFences);
  const scoreClean = d.score(withoutFences);
  assert.ok(
    scoreFenced < scoreClean * 0.60,
    `markdown fences даёт жёсткий штраф: ${scoreFenced.toFixed(3)} < ${scoreClean.toFixed(3)} × 0.60`,
  );
});
