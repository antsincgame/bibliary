...
/* ───── Iter 13.1: lazy upgrade contract (regression) ───── */

test("Lazy upgrade contract: legacy book.md без layoutVersion ДОЛЖЕН получать Versator", () => {
  /* Имитируем real legacy book.md из v0.7.x (без layoutVersion в frontmatter).
     ensureVersatorUpgrade в library-catalog-ipc.ts должен применять applyLayout
     к body. Этот тест защищает контракт: если frontmatter.layoutVersion
     отсутствует/0/меньше LAYOUT_VERSION — body конвертится. */
  const legacy = `---
title: "Legacy Book"
author: Автор
year: 2020
...
Энтропия — это мера хаоса в системе.

Внимание: важно понимать определения.`;

  const fmMatch = legacy.match(/^---\r?\n[\s\S]*?\r?\n---\r?\n?/);
  assert.ok(fmMatch, "frontmatter should be detected");
  const body = legacy.slice(fmMatch![0].length);
  const result = applyLayout(body, { lang: "ru" });

  /* Versator markers должны быть в body после lazy upgrade. */
  assert.match(result.md, /lib-reader-callout/, "warning callout expected");
  assert.match(result.md, /lib-reader-dropcap/, "drop cap expected");
  assert.match(result.md, /lib-reader-dfn/, "dfn expected");
  assert.equal(result.version, LAYOUT_VERSION);
});

test("Lazy upgrade contract: уже-обработанный body не дублирует Versator-разметку", () => {
  const md = `## Глава\n\nЭнтропия — это мера хаоса.\n\nВнимание: важная мысль.`;
  const once = applyLayout(md, { lang: "ru" }).md;
  const twice = applyLayout(once, { lang: "ru" }).md;
  /* Главное: не дублировать callout/dfn при повторном применении.
     Это критично для lazy upgrade — если он сработает дважды (race condition),
     book.md не должен быть изуродован. */
  assert.equal(
    (twice.match(/lib-reader-callout-warning/g) ?? []).length,
    (once.match(/lib-reader-callout-warning/g) ?? []).length,
    "callout-warning count must be stable",
  );
  assert.equal(
    (twice.match(/lib-reader-dfn/g) ?? []).length,
    (once.match(/lib-reader-dfn/g) ?? []).length,
    "dfn count must be stable",
  );
});
