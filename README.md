...
### 🔍 Поиск
- Семантический поиск по векторам в Qdrant (доступен через IPC `qdrant:search`,
  встроенного UI поиска нет — данные пишутся в Qdrant и доступны любому клиенту,
  включая официальный Qdrant Web UI на `http://localhost:6333/dashboard`)
...
- **Qdrant обязателен** — без него кристаллизация недоступна. Достаточно запустить через Docker.
- **OCR медленный** — сканированные DJVU/PDF с OCR обрабатываются медленно (минуты на книгу). Можно отключить OCR при импорте.
- **Поиск без встроенного UI** — приложение готовит данные и складывает в Qdrant. Для интерактивного поиска используйте Qdrant Web UI (`http://localhost:6333/dashboard`) или любой Qdrant-клиент. Встроенный UI поиска по семантике в roadmap.
