...
- **Путь A — Dataset:** book.md → semantic chunks → LLM-кристаллизатор → концепты в LanceDB → ChatML JSONL
- **Путь B — Search:** book.md → raw chunks → vector embeddings → LanceDB → семантический поиск по смыслу
...
Фильтрация качества  ←── отсевает беллетристику, воду, низкокачественные сканы
        │
        ▼ Кристаллизация
Смысловые чанки → LanceDB-коллекции  ←── векторизованные блоки по тематикам
        │
        ▼ Генерация датасета
...
| `crystallizing` | Главы бьются на смысловые чанки, чанки векторизуются и уходят в LanceDB. |
...
- Batch upsert в LanceDB — идемпотентно, resume-safe после краша

### 📊 Генерация датасета
- LanceDB-чанки → ChatML JSONL для fine-tuning LLM
- T1 / T2 / T3 уровни сложности на каждый чанк
- R1-style reasoning трейсы (опционально)
- Экспорт в файл

### 🔍 Семантический поиск (бонус)
- Поиск по векторам в LanceDB через тот же `multilingual-e5-small`
...
│       │   ├── ingest.ts           # parse → chunk → embed → LanceDB (Browse tab)
│       │   └── embedding.ts        # multilingual-e5-small wrapper
│       ├── dataset-v2/     # Кристаллизация + генерация ChatML JSONL
│       │   ├── extraction-runner.ts   # LanceDB-чанки → LLM → Q&A пары
...
│       ├── chroma/         # LanceDB клиент, управление коллекциями
...
[Пользователь выбирает папку или файлы]
 ...
 dataset-v2/extraction-runner.ts:            scanner/ingest.ts:
   ...
   chunkChapter() + cosine drift              Float32[384] → LanceDB upsert
   delta-extractor.ts:
     LLM(crystallizer) → концепты            ← Используется в семантическом поиске
   embed(essence) → LanceDB upsert
   upsertBook(status='indexed')
          │
          ▼ [раздел Crystal / Датасеты]
 dataset-v2/concept-loader.ts → LanceDB scroll(collection) → концепты
...
2. Запустите LM Studio, загрузите модель (рекомендуется 7–14B reasoning)
3. Запустите exe — векторная БД (LanceDB) встроена в приложение, данные хранятся рядом с exe
...
> Векторная БД (LanceDB) встроена в приложение и хранится локально в
> `userData/lancedb/`. Никаких отдельных URL'ов / токенов настраивать
> не нужно.
...
- Чанки векторизуются и уходят в LanceDB
- Статус → `indexed`
...
| `electron/lib/scanner/ingest.ts` | parse → chunk → embed → LanceDB (Browse path) |
| `electron/lib/dataset-v2/` | Кристаллизация + ChatML JSONL export |
| `electron/lib/resilience/` | Circuit breaker, bulkhead, AIMD, watchdog |
| `electron/lib/vectordb/` | In-process LanceDB adapter: connection, store, points, scroll, filter |
...
├── vectordb-*.test.ts         # In-process LanceDB adapter: store, points, scroll, filter
