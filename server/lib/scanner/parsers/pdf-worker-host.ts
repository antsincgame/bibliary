 ...
 * После tsc оба файла — в `dist-server/server/lib/scanner/parsers/`.
 *
 ...
 * в одной директории `dist-server/server/lib/scanner/parsers/`. Если файла
...
function resolveWorkerEntry(): string | null {
  ...
  const candidates = [
    path.join(process.cwd(), "dist-server", "server", "lib", "scanner", "parsers", "pdf-worker.js"),
  ];
