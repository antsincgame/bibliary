...
/**
 * Phase Round-3 library god-object split. Read-only catalog surface +
 * destructive book deletion. Caller (libraryRoutes) handles middleware.
 *
 * Endpoints:
 *   GET    /books                       paginated catalog with filters
 *   GET    /books/:id                   single book metadata
 *   GET    /books/:id/markdown          raw markdown bytes
 *   GET    /books/:id/cover             cover image bytes
 *   GET    /books/:id/original          original file bytes (Content-Disposition)
 *   DELETE /books/:id                   remove from catalog, optionally storage
 *   GET    /tag-stats                   aggregated tag counts (locale-aware)
 *   GET    /collection/by-domain        group by domain
 *   GET    /collection/by-author        group by author
 *   GET    /collection/by-year          group by year
 *   GET    /collection/by-tag           group by tag
 */
...
export function registerCatalogRoutes(app: Hono<AppEnv>): void {
