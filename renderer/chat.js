...
export function mountChat() {
  ...
  async function handleCreateCollection() {
    ...
    try {
      ...
      if (!result || result.ok === false) {
        ...
        await maybeOpenQdrantDashboard(errMsg);
        return;
      }
  ...
  /**
   * In-app input dialog instead of window.prompt (prompt can be suppressed or lose focus in Electron).
   * @returns {Promise<string | null>}
   */
  function promptCollectionName() {
  ...
  async function maybeOpenQdrantDashboard(errorMsg) {
